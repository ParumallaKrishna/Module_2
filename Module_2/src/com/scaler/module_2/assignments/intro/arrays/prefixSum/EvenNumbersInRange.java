...
public class EvenNumbersInRange {
	...
	public ArrayList<Integer> solve(ArrayList<Integer> A, ArrayList<ArrayList<Integer>> B) {
        ...
        for(int i=1;i<A.size();i++){
           if(A.get(i)%2==0){
               pcount.add(i,pcount.get(i-1)+1);
           }
           else{
        	   pcount.add(i,pcount.get(i-1));
           }
        }
        ArrayList<Integer> evenRange = new ArrayList<>();

        for(int j=0; j<B.size();j++){
            int L=B.get(j).get(0);
            int R=B.get(j).get(1);
            if(L==0){
                evenRange.add(j,pcount.get(R));
            }
            else {evenRange.add(j,pcount.get(R)-pcount.get(L-1));}
        }
        return evenRange;
